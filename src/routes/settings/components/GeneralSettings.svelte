<script lang="ts">
  ...
  function buildTree(flatInsights: Insight[]): TreeNode[] {
    const sorted = [...flatInsights].sort(
  ...
  async function removeFolder(path: string) {
    try {
      const config = await invoke<any>("get_config");
      config.watched_folders = config.watched_folders.filter(
        (f: string) => f !== path,
      );
      await invoke("set_config", { config });
      await invoke("refresh_repos");
    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3 min-w-0 flex-1">
          ...
          {/if}
          <div
            class="p-2.5 rounded-xl bg-white/5 text-muted-foreground group-hover:text-primary group-hover:bg-primary/5 transition-all duration-500"
          >
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p
                class="text-sm font-semibold truncate text-foreground/90 group-hover:text-foreground transition-colors leading-tight"
              >
                {node.folder.path.split("/").pop() || node.folder.path}
              </p>
              <div
                class="w-2 h-2 rounded-full {node.folder.scan_status ===
                'Synced'
                  ? 'bg-emerald-500 box-shadow-emerald'
                  : 'bg-amber-500 box-shadow-amber animate-pulse'}"
              ></div>
            </div>
      ...
      {#if node.folder.error_details}
        <div
          class="flex items-center text-amber-500/80 text-[10px] italic mt-2 pt-2 border-t border-white/5"
        >
...
<Card class="glass border-white/5 overflow-hidden">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <Button
          size="sm"
          onclick={addFolder}
          class="rounded-full bg-primary hover:scale-105 transition-transform px-4 border shadow-lg shadow-primary/20"
        >
  ...
  <CardContent class="pt-6">
    ...
    <div class="flex items-center gap-6 mb-8 px-2">
      <div class="flex items-center gap-2">
        <div
          class="w-2 h-2 rounded-full bg-emerald-500 box-shadow-emerald"
        ></div>
        <span
      ...
      <div class="flex items-center gap-2">
        <div
          class="w-2 h-2 rounded-full bg-amber-500 box-shadow-amber animate-pulse"
        ></div>
        <span
          class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider"
          >Scanning / Warning</span
        >
      </div>
    </div>
    ...
    {:else if insights.length === 0}
      <div
        class="py-20 border-2 border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center space-y-4 text-muted-foreground bg-white/[0.01]"
      >
