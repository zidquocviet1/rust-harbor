<script lang="ts">
  ...
  import { Hash, PlusCircle, Save, ShieldCheck, X } from "lucide-svelte";
    ...
    <Card class="glass border-white/5">
      <CardContent class="p-8 space-y-8">
        <div class="px-4 flex items-center justify-between">
          <div class="space-y-1">
            ...
            <CardDescription class="text-sm"
              >Configure how {APP_BRANDING.shortName} traverses your filesystem.</CardDescription
            >
          </div>

          <div
            class="flex items-center gap-4 px-4 py-2 bg-white/[0.02] border border-white/5 rounded-2xl"
          >
            ...
            <div class="flex items-center gap-1.5">
              <div
                class="w-2 h-2 rounded-full bg-white/20 border border-white/10"
              ></div>
              <span
                class="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60"
                >User</span
        ...
        <div class="space-y-4">
          ...
          >
            <div class="space-y-0.5">
              ...
              <p class="text-[10px] text-muted-foreground opacity-60">
                Maximum directory nesting level for repository discovery.
              </p>
            </div>
            <div class="flex items-center gap-4">
              ...
              />
              <div
                class="flex items-center justify-center w-10 h-10 rounded-xl bg-primary/10 border border-primary/20 text-primary font-mono text-sm font-bold"
              >
                {config.max_depth}
              </div>
            </div>
          ...
          <div
            class="space-y-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5"
          >
            <div class="space-y-0.5">
              <Label class="text-sm font-semibold">Exclusion Patterns</Label>
              <p class="text-[10px] text-muted-foreground opacity-60">
                Glob patterns of heavy or irrelevant directories to ignore.
              </p>
            </div>
            ...
            <div class="flex gap-2">
              ...
              <Button
                size="icon"
                class="rounded-xl h-11 w-11 bg-primary hover:scale-105 transition-transform border border-primary/20 shadow-lg shadow-primary/10"
                onclick={addPattern}
              >
            ...
            <div class="flex flex-wrap gap-2 pt-2">
              {#each config.exclusion_patterns as pattern}
                ...
                <div
                  class="flex items-center gap-2 px-3 py-2 rounded-xl transition-all duration-300 {isSystem
                    ? 'bg-white/[0.03] border-white/5 opacity-60'
                    : 'bg-primary/5 border-primary/20 text-primary'} border text-[11px] font-mono group"
                >
                  {#if isSystem}
                  ...
                  <span
                    class={isSystem
                      ...
                      : "text-primary font-medium"}>{pattern}</span
                  >
        ...
        <div class="px-4">
          <Button
            ...
            class="w-full py-6 rounded-2xl bg-primary text-primary-foreground hover:scale-[1.01] active:scale-[0.99] transition-all shadow-xl shadow-primary/20 font-bold"
          >
