<script lang="ts">
  ...
  async function openPreview(repo: RepoMetadata) {
    ...
    const load = async () => {
      try {
        ...
        const response = await invoke<{ html: string; raw: string }>(
          "get_repo_readme",
          { path: repo.path },
        );
  ...
  function getRepoLanguageInfo(
    repo: RepoMetadata | null | undefined,
  ): RepoLanguageInfo {
  ...
  function recalcLanguageVisibleCount() {
    ...
    const allChip = languageMeasureEl.querySelector(
      "[data-all-chip]",
    ) as HTMLElement | null;
    const langChips = Array.from(
      languageMeasureEl.querySelectorAll("[data-lang-chip]"),
    ) as HTMLElement[];
    const moreChip = languageMeasureEl.querySelector(
      "[data-more-chip]",
    ) as HTMLElement | null;
  ...
  const filteredRepos = $derived.by(() => {
    ...
    if (selectedLanguages.length > 0) {
      result = result.filter((r) =>
        selectedLanguages.some((lang) =>
          Object.keys(r.languages || {}).includes(lang),
        ),
      );
    }
    ...
    if (selectedTagNames.length > 0) {
      result = result.filter(
        (r) => r.tags && r.tags.some((tag) => selectedTagNames.includes(tag)),
      );
    }
  ...
  $effect(() => {
    ...
    const nextRepos = repos.map((repo) => {
      const filteredTags = (repo.tags || []).filter((tag) =>
        validTagNames.has(tag),
      );
  ...
  function getSyncStatusDetails(status: SyncStatus) {
    switch (status) {
      ...
      case "Ahead":
        return { icon: ArrowUp, color: "text-primary", label: "Ahead" };
      case "Behind":
        return { icon: ArrowDown, color: "text-amber-500", label: "Behind" };
      case "Dirty":
        return {
          icon: GitCommit,
      ...
      case "Conflict":
        return { icon: GitMerge, color: "text-destructive", label: "Conflict" };
      default:
        return {
          icon: AlertTriangle,
  ...
  function getEditorIcon(iconName: string): string | null {
    const key =
      `si${iconName.charAt(0).toUpperCase()}${iconName.slice(1).toLowerCase()}` as keyof typeof si;
  ...
  async function runGitAction(
    ...
    toast.promise(promise, {
      ...
      error: (e: any) =>
        `Git ${action} failed: ${typeof e === "string" ? e : e.GitError || e.IoError || "Unknown error"}`,
    });
    try {
      await promise;
    } finally {
  ...
  onMount(async () => {
    ...
    const loadTagsPromise = withTimeout(loadTags(), 8000, "load_tags").catch(
      (e) => {
  ...
  async function toggleTagForRepo(repo: RepoMetadata, tag: StoreTag) {
    try {
      ...
      const updated = await invoke<StoreTag[]>("get_repo_tags", {
        repoPath: repo.path,
      });
      const tagNames = updated.map((t) => t.name);

      repos = repos.map((r) =>
        r.path === repo.path ? { ...r, tags: tagNames } : r,
      );
      ...
      const message =
        typeof e === "string" ? e : e?.message || "Failed to update tag";
      toast.error(message);
    }
  }
</script>
...
{#snippet RepoCard({
  ...
  >
      ...
      <CardContent class="p-8 space-y-8 flex-1 flex flex-col">
        <div class="flex items-start justify-between gap-6">
          <div class="space-y-2 flex-1 min-w-0">
            <h3
              class="text-2xl font-black tracking-tight truncate group-hover:text-primary transition-colors"
            >
              {repo.name}
            </h3>
          </div>
          <div
            class="p-3 bg-white/80 rounded-2xl text-muted-foreground group-hover:text-primary group-hover:bg-primary/10 transition-all duration-500"
          >
        ...
        <div class="flex flex-col gap-3">
          <div class="flex flex-wrap gap-1.5 min-h-[24px]">
            {#each langInfo.sortedLanguages.slice(0, 3) as [lang, count]}
              ...
              <Badge
                variant="outline"
                class="bg-white/80 border-slate-200/70 text-[8px] px-2 py-0.5 rounded-md font-black uppercase tracking-widest text-muted-foreground"
              >
                {#if icon}
                  <svg
                    viewBox="0 0 24 24"
                    class="w-2.5 h-2.5 mr-1 inline-block align-[-2px]"
            ...
            {#if langInfo.languageCount > 3}
              <Badge
                variant="outline"
                class="bg-white/80 border-slate-200/70 text-[8px] px-2 py-0.5 rounded-md font-black"
              >
          ...
          <div class="flex flex-wrap gap-1.5 min-h-[20px] mb-2">
            ...
            {#if repo.tags.length > 3}
              <Badge
                variant="outline"
                class="bg-white/80 border-slate-200/80 text-[8px] px-2 py-0.5 rounded-full font-black"
              >
                +{repo.tags.length - 3}
              </Badge>
            {/if}
          ...
          {#if repo.description}
            <p
              class="text-[13px] text-muted-foreground line-clamp-3 min-h-[3rem] leading-relaxed font-medium"
            >
              {repo.description}
            </p>
          {/if}
        ...
        <div class="flex flex-wrap gap-2 pt-2">
          <Badge
            variant="outline"
            class="bg-white/80 border-slate-200/70 text-[10px] px-3 py-1 rounded-full font-bold"
          >
        ...
        <div
          class="mt-auto pt-8 flex items-center justify-between border-t border-slate-200/70 relative"
        >
          <div
            class="flex items-center bg-white/85 rounded-full p-1.5 border border-slate-200/70 shadow-inner"
          >
            <Button
              variant="ghost"
              size="icon"
              class="rounded-full h-11 w-11 transition-colors duration-200 hover:bg-emerald-50 hover:text-emerald-600"
            ...
            >
              <RefreshCw
                class="w-5 h-5 {actionLoading[`${repo.path}-fetch`]
                  ? 'animate-spin'
                  : ''}"
              />
            </Button>
            <Button
              variant="ghost"
              size="icon"
              class="rounded-full h-11 w-11 transition-colors duration-200 hover:bg-sky-50 hover:text-sky-600"
            ...
            >
              <ArrowDown
                class="w-5 h-5 {actionLoading[`${repo.path}-pull`]
                  ? 'animate-bounce'
                  : ''}"
              />
            </Button>
            <Button
              variant="ghost"
              size="icon"
              class="rounded-full h-11 w-11 transition-colors duration-200 hover:bg-violet-50 hover:text-violet-600"
            ...
            >
              <ArrowUp
                class="w-5 h-5 {actionLoading[`${repo.path}-push`]
          ...
          <div class="flex items-center space-x-1">
            ...
            <Button
              ...
              onmouseleave={() => (hoveredAction = null)}
              onclick={(e) =>
                openTagPopover(repo, e.currentTarget as HTMLElement)}
            >
              <Tag class="w-4 h-4" />
            </Button>
          </div>
    ...
    {:else}
      <!-- List Item -->
      <div
        class="flex-1 flex items-center justify-between py-3 px-5 overflow-hidden gap-6"
      >
        <div class="flex items-center space-x-4 min-w-0 flex-1">
          <div
            class="p-2.5 bg-white/80 rounded-2xl text-muted-foreground group-hover:text-primary transition-all duration-500 group-hover:bg-primary/10"
          >
            <GitBranch class="w-5 h-5" />
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center space-x-3 mb-2">
              <h3 class="font-bold truncate text-[15px] tracking-tight">
                {repo.name}
              </h3>
              <div class="flex gap-1.5">
                {#each langInfo.sortedLanguages.slice(0, 2) as [lang]}
                  {@const icon = getLanguageIcon(lang)}
                  <span
                    class="inline-flex items-center text-[11px] px-2 py-0.5 bg-white/80 border border-slate-200/70 text-muted-foreground font-bold uppercase tracking-wide rounded-md"
                  >
                    {#if icon}
                      <svg
                        viewBox="0 0 24 24"
            ...
            {#if repo.description}
              <p
                class="text-[13px] text-muted-foreground font-medium truncate leading-snug mb-2"
              >
                {repo.description}
              </p>
            {/if}
            ...
            {#if repo.tags.length}
              <div class="flex flex-wrap gap-1.5 mt-1">
                ...
                {#if repo.tags.length > 3}
                  <span
                    class="text-[11px] px-2 py-0.5 rounded-full bg-white/80 border border-slate-200/80 font-bold text-muted-foreground"
                  >
                    +{repo.tags.length - 3}
                  </span>
                {/if}
        ...
        <div class="flex items-center gap-6 flex-shrink-0">
          <div class="w-28 flex flex-col items-center gap-1">
            ...
            >
            <span
              class="text-sm font-bold tracking-tight truncate max-w-[104px]"
              >{repo.branch}</span
            >
          </div>
          <div class="w-28 flex flex-col items-center gap-1">
            ...
            <div class="flex items-center space-x-1.5">
              ...
              <span class="text-sm font-bold tracking-tight {status.color}"
                >{status.label}</span
              >
            </div>
          </div>
          <div class="w-28 flex flex-col items-center gap-1">
            <span
              ...
              >Activity</span
            >
            <span class="text-sm font-bold text-muted-foreground tracking-tight"
        ...
        <div class="flex items-center space-x-2 flex-shrink-0 relative">
          ...
          <Button
            variant="ghost"
            size="icon"
            class="rounded-xl h-11 w-11 transition-colors duration-200 hover:bg-emerald-50 hover:text-emerald-600"
            onmouseenter={(e) => hoverAction(e, repo.path, "Fetch")}
            onmouseleave={() => (hoveredAction = null)}
            disabled={!!actionLoading[`${repo.path}-fetch`]}
            onclick={() => runGitAction(repo, "fetch")}
          ...
          <Button
            variant="ghost"
            size="icon"
            class="rounded-xl h-11 w-11 transition-colors duration-200 hover:bg-sky-50 hover:text-sky-600"
            onmouseenter={(e) => hoverAction(e, repo.path, "Pull")}
            onmouseleave={() => (hoveredAction = null)}
            disabled={!!actionLoading[`${repo.path}-pull`]}
            onclick={() => runGitAction(repo, "pull")}
          ...
          <Button
            variant="ghost"
            size="icon"
            class="rounded-xl h-11 w-11 transition-colors duration-200 hover:bg-amber-50 hover:text-amber-600"
            onmouseenter={(e) => hoverAction(e, repo.path, "Explore")}
            onmouseleave={() => (hoveredAction = null)}
            onclick={() => openFolder(repo.path)}
          >
...
<div class="p-8 space-y-8 max-w-7xl mx-auto">
  ...
  <div class="space-y-5">
    <div>
      ...
      <p class="text-muted-foreground text-xl">
        Managing <span class="text-primary font-bold">{repos.length}</span> repositories.
      </p>
    </div>

    <div
      class="flex flex-nowrap items-center gap-4 overflow-x-hidden overflow-y-visible"
    >
      <div class="flex flex-nowrap items-center gap-3 flex-1 min-w-0">
        ...
        >
          <div
            class="flex flex-nowrap items-center gap-1.5 overflow-hidden min-w-0 flex-1"
          >
            <button
              onclick={() => (selectedLanguages = [])}
              class="px-3.5 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all border border-transparent shrink-0 {selectedLanguages.length ===
              0
                ? 'bg-primary text-primary-foreground shadow-glow border-primary/20'
            ...
            {#each visibleLanguages as lang}
              ...
              <button
                ...
                class="px-3.5 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all border whitespace-nowrap shrink-0 {selectedLanguages.includes(
                  lang.name,
                )
                  ? 'bg-white/10 text-primary border-primary/40 shadow-glow'
                  : 'hover:bg-slate-100 text-muted-foreground border-transparent'}"
              >
                <div class="flex items-center gap-2">
                  {#if icon}
                    <svg
                      ...
                      style={`color: #${icon.hex}`}
                      aria-label={`${lang.name} icon`}
                    >
                      <path fill="currentColor" d={icon.path}></path>
                    </svg>
                  {/if}
                  {lang.name}
                  <span class="opacity-40 text-[8px] font-medium"
                    >{lang.count}</span
                  >
          ...
          {#if hiddenLanguages.length > 0}
            <div class="shrink-0">
              <button
                ...
                class="px-3.5 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all border whitespace-nowrap {hiddenLanguages.some(
                  (l) => selectedLanguages.includes(l.name),
                )
        ...
        {#if showLanguageDropdown && languageDropdownPosition}
          ...
          >
            ...
            <div
              class="max-h-64 overflow-y-auto space-y-1 custom-scrollbar pr-1"
            >
              {#each hiddenLanguages as lang}
                ...
                <button
                  ...
                  class="w-full text-left px-3 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 transition-all flex items-center justify-between group {selectedLanguages.includes(
                    lang.name,
                  )
                ...
                >
                  ...
                  {:else}
                    <div
                      class="w-1.5 h-1.5 rounded-full bg-white/80 group-hover:bg-primary/40 transition-colors"
                    ></div>
                  {/if}
                </button>
              {/each}
        ...
        >
          <div class="flex flex-nowrap items-center gap-1.5">
            <button
              data-all-chip
              class="px-3.5 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg border border-transparent"
            >
              All
            </button>
            {#each langStats as lang}
              ...
              <button
                data-lang-chip
                class="px-3.5 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg border whitespace-nowrap"
              >
                <div class="flex items-center gap-2">
                  {#if icon}
                    <svg
      ...
      <div class="flex items-center gap-3 shrink-0 ml-auto">
        ...
        <div
          class="bg-white/80 rounded-xl border border-slate-200/80 p-1 flex items-center gap-1"
        >
          <span
            class="px-2 text-[9px] uppercase tracking-[0.25em] text-muted-foreground"
            >Group By</span
          >
          <button
            onclick={() => setGroupByMode("none")}
            class="px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all {groupByMode ===
            'none'
              ? 'bg-white/10 text-primary shadow-inner'
              : 'text-muted-foreground hover:bg-slate-100'}"
          >
  ...
  {#if loading && repos.length === 0}
    ...
    <div class="py-32 flex flex-col items-center justify-center space-y-6">
      ...
      <p
        class="text-sm font-black uppercase tracking-[0.2em] text-primary/60 animate-pulse"
      >
        Scanning Horizon
      </p>
    </div>
  {:else if filteredRepos.length === 0}
    <div
      class="py-32 glass rounded-3xl flex flex-col items-center justify-center space-y-8 text-center px-6"
    >
      <div
        class="relative p-8 rounded-full bg-white/80 border border-slate-200/80"
      >
  ...
  >
    <div class="flex items-center justify-between mb-1">
      <div class="space-y-0.5">
        <p
          class="text-[10px] uppercase tracking-[0.25em] text-muted-foreground font-black"
        >
          Tags
        </p>
    ...
    {:else}
      <div class="max-h-64 overflow-y-auto space-y-1 pr-1">
        {#each $allTags as tag}
          ...
          <button
            class="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-[11px] hover:bg-slate-100 border border-transparent {checked
              ? 'text-foreground'
              : ''}"
            style={checked ? `background-color: ${tag.color}22` : ""}
            onclick={() =>
              tagPopoverRepo && toggleTagForRepo(tagPopoverRepo, tag)}
          >
...
{#if selectedRepoForPreview}
  ...
  >
    <!-- Header -->
    <div
      class="p-6 border-b border-slate-200/70 flex items-center justify-between bg-white/80"
    >
      <div class="space-y-1">
        <h2 class="text-xl font-black uppercase tracking-widest">
          {selectedRepoForPreview.name}
        </h2>
      </div>
      <Button
        variant="ghost"
        ...
        >
          ...
          {:else}
            <div class="space-y-10 animate-in fade-in duration-200 pb-20">
              <div class="space-y-4">
                <h3
                  class="text-xs font-black uppercase tracking-[0.2em] text-primary border-l-2 border-primary pl-4"
                >
                  Metadata Summary
                </h3>
                <div class="grid grid-cols-2 gap-4">
                  <div
                    class="bg-white/90 p-5 rounded-2xl border border-slate-200/70"
                  >
                    <p
                      class="text-[9px] text-muted-foreground uppercase font-black mb-1.5 tracking-widest"
                    >
                      Active Branch
                    </p>
                    <div class="flex items-center gap-2.5">
                      ...
                      <span class="text-sm font-bold tracking-tight"
                        >{selectedRepoForPreview.branch}</span
                      >
                    </div>
                  </div>
                  <div
                    ...
                    <div class="flex items-center gap-2.5">
                      ...
                      <span class="text-sm font-bold tracking-tight"
                        >{getSyncStatusDetails(
                          selectedRepoForPreview.sync_status,
                        ).label}</span
                      >
                    </div>
                  </div>
              ...
              <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-2.5">
                  {#each getRepoLanguageInfo(selectedRepoForPreview).sortedLanguages as [lang, count]}
                    {@const icon = getLanguageIcon(lang)}
                    <div
                      class="bg-white/90 px-5 py-3 rounded-2xl border border-slate-200/70 flex items-center gap-4 hover:border-primary/20 transition-colors"
                    >
                      {#if icon}
                        <svg
                          viewBox="0 0 24 24"
                      ...
                      >
                      <span
                        class="text-[9px] font-black p-1 bg-white/80 rounded border border-slate-200/70 text-muted-foreground"
                        >{count}</span
                      >
                    </div>
                  {/each}
              ...
              {#if selectedRepoForPreview.description}
                <div class="space-y-4">
                  ...
                  <div
                    class="bg-primary/5 p-6 rounded-3xl border border-primary/10 italic text-sm text-muted-foreground leading-relaxed"
                  >
              ...
              <div class="space-y-4">
                ...
                <div
                  class="bg-white/90 p-5 rounded-2xl border border-slate-200/70 flex items-center justify-between"
                >
                  <div class="flex items-center gap-3 min-w-0">
                    <FolderOpen class="w-4 h-4 text-muted-foreground" />
                    <span
                      class="text-xs font-mono text-muted-foreground truncate max-w-xs"
                      >{selectedRepoForPreview.path}</span
                    >
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                >
                  ...
                  {#if selectedRepoForPreview.remote_url}
                    <Badge
                      variant="outline"
                      class="{selectedRepoForPreview.remote_reachable
                        ? 'bg-emerald-500/10 text-emerald-500'
                        : 'bg-destructive/10 text-destructive'} border-transparent"
                    >
                      {selectedRepoForPreview.remote_reachable
                        ? "Online"
                        : "Offline"}
                    </Badge>
                  {/if}
...
{#if repoContextMenuRepo && repoContextMenuPosition}
  ...
  >
    ...
    <button
      ...
      onclick={() => {
        repoContextMenuRepo && openFolder(repoContextMenuRepo.path);
        closeRepoContextMenu();
      }}
    >
    ...
    <div
      class="overflow-y-auto max-h-[180px] overscroll-contain [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full"
    >
      {#each installedEditors as editor}
        <button
          ...
          onclick={() =>
            repoContextMenuRepo &&
            openInEditor(editor.id, repoContextMenuRepo.path)}
        >
          <div
            class="w-4 h-4 flex items-center justify-center opacity-50 group-hover:opacity-100 shrink-0"
          >
            {#if getEditorIcon(editor.icon)}
              <div class="w-3.5 h-3.5 fill-current">
                {@html getEditorIcon(editor.icon)}
              </div>
            {:else}
              <Code2
                class="w-3.5 h-3.5 text-slate-400 group-hover:text-primary"
              />
            {/if}
          </div>
          <span class="font-medium text-slate-700 group-hover:text-primary"
    ...
    <button
      class="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-primary/5 text-left transition-colors group"
      onclick={() => {
        repoContextMenuRepo && runGitAction(repoContextMenuRepo, "fetch");
        closeRepoContextMenu();
      }}
    >
