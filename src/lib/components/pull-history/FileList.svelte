<script lang="ts">
  ...
  import { ChevronRight, Folder, FolderOpen, FileText, Loader2 } from "lucide-svelte";
  ...
  let { files, totalAdditions, totalDeletions }: Props = $props();
  ...
  // ── Tree construction ─────────────────────────────────────────────────────────
  function insertFile(parent: DirNode, file: PullHistoryFile, parts: string[], depth: number) {
    if (depth === parts.length - 1) {
      parent.children.push({ type: "file", name: parts[depth], path: file.file_path, file, additions: file.additions, deletions: file.deletions, fileCount: 1 });
      return;
    }
    ...
    if (!dir) {
      dir = { type: "dir", name: dirName, path: dirPath, children: [], additions: 0, deletions: 0, fileCount: 0 };
  ...
  function computeStats(node: DirNode) {
    node.additions = 0; node.deletions = 0; node.fileCount = 0;
    for (const child of node.children) {
      if (child.type === "file") { node.additions += child.additions; node.deletions += child.deletions; node.fileCount++; }
      else { computeStats(child); node.additions += child.additions; node.deletions += child.deletions; node.fileCount += child.fileCount; }
    }
  }
  ...
  function collectDirPaths(nodes: TreeNode[]): string[] {
    return nodes.flatMap((n) => n.type === "dir" ? [n.path, ...collectDirPaths(n.children)] : []);
  }

  const tree = $derived.by(() => {
    const root: DirNode = { type: "dir", name: "", path: "", children: [], additions: 0, deletions: 0, fileCount: 0 };
    for (const file of files) insertFile(root, file, file.file_path.split("/"), 0);
    computeStats(root);
    sortChildren(root);
  ...
  async function toggleFile(file: PullHistoryFile) {
    ...
    if (!diffCache.has(id) && file.change_type !== "binary" && file.change_type !== "renamed") {
  ...
  const BADGE: Record<string, { label: string; cls: string; dot: string }> = {
    added:    { label: "Added",    cls: "bg-emerald-50 text-emerald-700 border-emerald-200/70", dot: "bg-emerald-500" },
    modified: { label: "Modified", cls: "bg-primary/8 text-primary border-primary/20",          dot: "bg-primary"    },
    deleted:  { label: "Deleted",  cls: "bg-red-50 text-red-700 border-red-200/70",             dot: "bg-red-500"    },
    renamed:  { label: "Renamed",  cls: "bg-amber-50 text-amber-700 border-amber-200/70",       dot: "bg-amber-500"  },
    binary:   { label: "Binary",   cls: "bg-muted text-muted-foreground border-border",         dot: "bg-muted-foreground" },
  };

  function badge(ct: string) { return BADGE[ct] ?? BADGE.modified; }

  // Icon colour by change type
  const FILE_ICON_CLS: Record<string, string> = {
    added: "text-emerald-500", modified: "text-primary", deleted: "text-red-400",
    renamed: "text-amber-500", binary: "text-muted-foreground",
  };
</script>
...
<div class="bg-white/80 rounded-2xl border border-slate-200/70 overflow-hidden">
  ...
  <div class="px-4 py-2.5 border-b border-slate-100 flex items-center justify-between">
    ...
    <div class="flex items-center gap-1.5">
      {#if totalAdditions > 0}
        <span class="text-[11px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/70">
          +{totalAdditions}
        </span>
      {/if}
      {#if totalDeletions > 0}
        <span class="text-[11px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-200/70">
          -{totalDeletions}
        </span>
      {/if}
    </div>
  ...
  {#if node.type === "dir"}
    ...
    >
      <ChevronRight
        size={12}
        strokeWidth={2.5}
        class="shrink-0 text-muted-foreground transition-transform duration-140 {collapsed ? '' : 'rotate-90'}"
      />
      ...
      <span class="flex-1 font-mono text-[12px] font-semibold min-w-0 truncate text-foreground">
        {node.name}/
      </span>

      <!-- File count pill -->
      <span class="shrink-0 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground border border-border">
        {node.fileCount} {node.fileCount === 1 ? "file" : "files"}
      </span>

      <!-- Stat badges -->
      {#if node.additions > 0}
        <span class="shrink-0 text-[11px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/70">
          +{node.additions}
        </span>
      {/if}
      {#if node.deletions > 0}
        <span class="shrink-0 text-[11px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-200/70">
          -{node.deletions}
        </span>
      {/if}
    </button>
    ...
    {#if !collapsed}
      <div class="ml-5 pl-3 border-l-2 border-slate-200 flex flex-col gap-px my-px">
    ...
    <div>
      ...
      >
        ...
        <FileText
          size={12}
          strokeWidth={2.5}
          class="shrink-0 {FILE_ICON_CLS[node.file.change_type] ?? 'text-muted-foreground'}"
        />

        <!-- Filename -->
        {#if node.file.change_type === "renamed"}
          <span class="flex-1 font-mono text-[11.5px] min-w-0 truncate text-muted-foreground">
            {node.file.file_path.split(" → ")[0]?.split("/").at(-1) ?? node.name}
        ...
        <!-- Change type badge -->
        <span class="shrink-0 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold border {badge(node.file.change_type).cls}">
          <span class="w-1 h-1 rounded-full {badge(node.file.change_type).dot}"></span>
        ...
        {#if node.file.change_type !== "binary" && node.file.change_type !== "renamed"}
          {#if loadingDiffs.has(node.file.id)}
            <Loader2 size={11} strokeWidth={2.5} class="shrink-0 text-muted-foreground animate-spin" />
          {:else}
            <ChevronRight
              size={11}
              strokeWidth={2.5}
              class="shrink-0 text-muted-foreground transition-transform duration-140 {expandedFiles.has(node.file.id) ? 'rotate-90' : ''}"
            />
          {/if}
        {/if}
      </button>
